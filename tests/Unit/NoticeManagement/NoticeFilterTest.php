...
class NoticeFilterTest extends TestCase {
	...
	protected function setUp(): void {
		...
		// Mock get_option for global toggle (default to 'no' = enabled for filtering)
		Functions\when( 'get_option' )->alias(
			function ( $option, $default = false ) {
				if ( $option === 'qala_notices_enabled' ) {
					  return 'no'; // 'no' means filtering is enabled (nuclear mode active)
				}
				return $default;
			}
		);
	...
	/**
	 * Test: filter_notices() skips if global toggle is enabled (shows all notices)
	 *
	 * @return void
	 */
	public function test_filter_notices_skips_when_global_toggle_enabled(): void {
		// Mock global toggle as 'yes' (shows all notices, filtering disabled)
		...
		$this->assertTrue( true, 'Should skip filtering when global toggle is enabled (yes = show all)' );
	}
