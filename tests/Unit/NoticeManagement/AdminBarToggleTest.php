...
class AdminBarToggleTest extends TestCase {
	...
	public function test_init_registers_enqueue_scripts_hook(): void {
		Actions\expectAdded( 'admin_enqueue_scripts' )
			->once()
			->with( [ $this->admin_bar_toggle, 'enqueue_assets' ], 5 );
	...
	/**
	 * Test: add_admin_bar_menu() shows "Notices: Hidden - Click to Show" when notices are hidden
	 *
	 * @return void
	 */
	public function test_add_admin_bar_menu_shows_off_state_when_hidden(): void {
		...
		$wp_admin_bar->shouldReceive( 'add_node' )
			...
			->with(
				Mockery::on(
					function ( $args ) {
						// Title should indicate notices are hidden
						return strpos( $args['title'], 'Hidden' ) !== false
						|| strpos( $args['title'], 'hidden' ) !== false;
					}
				)
			);
	...
	/**
	 * Test: add_admin_bar_menu() shows "Notices: Showing - Click to Hide" when notices are visible
	 *
	 * @return void
	 */
	public function test_add_admin_bar_menu_shows_on_state_when_visible(): void {
		...
		$wp_admin_bar->shouldReceive( 'add_node' )
			...
			->with(
				Mockery::on(
					function ( $args ) {
						// Title should indicate notices are showing
						return strpos( $args['title'], 'Showing' ) !== false
						|| strpos( $args['title'], 'showing' ) !== false;
					}
				)
			);
	...
	public function test_enqueue_assets_enqueues_javascript(): void {
		// Mock Plugin static methods
		$plugin_path = '/path/to/plugin';
		$plugin_url  = 'http://example.com/wp-content/mu-plugins/qala-plugin-manager';

		// Use Mockery to mock static methods
		$plugin_mock = Mockery::mock( 'alias:QalaPluginManager\Plugin' );
		$plugin_mock->shouldReceive( 'get_path' )->andReturn( $plugin_path );
		$plugin_mock->shouldReceive( 'get_url' )->andReturn( $plugin_url );
	...
	public function test_handle_toggle_ajax_returns_correct_title_when_toggling_to_visible(): void {
		...
		Functions\expect( 'wp_send_json_success' )
			...
			->with(
				Mockery::on(
					function ( $data ) {
						// New title should contain state indicator (On/Showing/etc)
	...
	public function test_handle_toggle_ajax_returns_correct_title_when_toggling_to_hidden(): void {
		...
		Functions\expect( 'wp_send_json_success' )
			...
			->with(
				Mockery::on(
					function ( $data ) {
						// New title should contain state indicator and showing should be false
						return isset( $data['new_title'] )
						&& isset( $data['showing'] )
						&& $data['showing'] === false;
					}
