...
class AdminBarToggle implements WithHooksInterface {
	...
	public function enqueue_assets(): void {
		$plugin_path = \QalaPluginManager\Plugin::get_path();
		$plugin_url = \QalaPluginManager\Plugin::get_url();

		$js_path = $plugin_path . '/assets/dist/js/qala-plugin-manager.js';
		$js_url = $plugin_url . '/assets/dist/js/qala-plugin-manager.js';
		$asset_file = $plugin_path . '/assets/dist/js/qala-plugin-manager.asset.php';

		$css_path = $plugin_path . '/assets/dist/qala-plugin-manager.css';
		$css_url = $plugin_url . '/assets/dist/qala-plugin-manager.css';

		// Debug logging
		error_log( 'Qala Plugin Manager: Enqueuing assets' );
		...
		error_log( 'Asset file: ' . $asset_file . ' (exists: ' . ( file_exists( $asset_file ) ? 'yes' : 'no' ) . ')' );

		// Load asset file for dependencies (generated by @wordpress/scripts)
		$asset_data = file_exists( $asset_file ) ? include $asset_file : [
			'dependencies' => [ 'jquery' ],
			'version' => '1.0.0',
		];

		// Enqueue bundled JavaScript (includes all JS functionality)
		wp_enqueue_script(
			'qala-plugin-manager',
			$js_url,
		...
		// Enqueue bundled CSS (includes all styles)
		wp_enqueue_style(
			'qala-plugin-manager',
			$css_url,
			[],
			file_exists( $css_path ) ? filemtime( $css_path ) : $asset_data['version']
		);
